...
1. Instale [MATLAB (MathWorks)](https://www.mathworks.com/help/install/ug/install-products-with-internet-connection.html) R2021a o posterior y agréguelo al PATH del sistema. MATLAB MCP Server es compatible con las versiones de MATLAB de los últimos cinco años.
1. El servidor es compatible con cualquier aplicación de IA que utilice Model Context Protocol. Para configurar MATLAB MCP Server para Claude Desktop, vaya directamente a las instrucciones de [Claude Desktop](#claude-desktop). Para configurar el servidor para otras aplicaciones, siga estas instrucciones:

   - Para Windows o Linux, [**descargue la última versión**](https://github.com/matlab/matlab-mcp-server/releases/latest). (Alternativamente, puede **compilar desde el código fuente**: instale [Go](https://go.dev/doc/install) y compile el binario con `go install github.com/matlab/matlab-mcp-server/cmd/matlab-mcp-server@latest`).
...
```sh
claude mcp add --transport stdio matlab -- /rutacompleta/al/binario-de-matlab-mcp-server
```
...
```sh
claude mcp add --transport stdio matlab -- /rutacompleta/al/binario-de-matlab-mcp-server --initial-working-folder=/home/username/myproject
```
...
Instale MATLAB MCP Server en Claude Desktop mediante el paquete MATLAB MCP Server.
...
2. Descargue el paquete MATLAB MCP Server `matlab-mcp-server.mcpb` desde la página de la [última versión](https://github.com/matlab/matlab-mcp-server/releases/latest). 

3. Para instalar el paquete MATLAB MCP Server como extensión de escritorio, haga doble clic en el archivo `matlab-mcp-server.mcpb` descargado y haga clic en **Install** en Claude Desktop. (Alternativamente, navegue en Claude a **File menu > Settings > Extensions > Advanced Settings > Install Extension** y seleccione el archivo `matlab-mcp-server.mcpb`. Haga clic en **Install**).

Para personalizar el comportamiento de MATLAB MCP Server, navegue a **Settings > Extensions > Configure**, donde puede modificar los [argumentos](#argumentos) del servidor.
...
```json
{
    "servers": {
        "matlab": {
            "type": "stdio",
            "command": "C:\\rutacompleta\\a\\matlab-mcp-server-windows-x64.exe",
            "args": []
        }
    }
}
...
| version | Muestra la versión de MATLAB MCP Server. | `--version` |
