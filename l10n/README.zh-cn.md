...
> [!WARNING]
> 自 v0.11.0 起，MATLAB MCP Core Server 已更名为 MATLAB MCP Server。在此变更之后，若要使用最新版本的服务器，必须更新您的设置。
>
> | 变更 | 所需操作 |
> |:-------:|:---------------:|
   ...
   - 对于 macOS，请先在终端中运行以下命令以下载最新版本:
     - 对于 Apple silicon 处理器，请运行:
      ...
      - 对于 Intel 处理器，请运行:
          ...
          ```
      然后向下载的二进制文件授予可执行权限，以便您可以运行 MATLAB MCP Server:
...
您可以使用 MATLAB MCP Server 捆绑包在 Claude Desktop 中安装 MATLAB MCP Server。
...
2. 从 [Latest Release](https://github.com/matlab/matlab-mcp-server/releases/latest) 页面下载 MATLAB MCP Server 捆绑包 `matlab-mcp-server.mcpb`。 

3. 要将 MATLAB MCP Server 捆绑包安装为桌面扩展程序，请双击下载的 `matlab-mcp-server.mcpb` 文件，然后在 Claude Desktop 中点击 **Install**。(或者，在 Claude 中导航到 **File 菜单 > Settings > Extensions > Advanced Settings > Install Extension**，然后选择 `matlab-mcp-server.mcpb` 文件。点击 **Install**)。

要自定义 MATLAB MCP Server 的行为，请导航到 **Settings > Extensions > Configure**，您可以在此修改服务器的[参量](#参量)。
...
MathWorks 鼓励您使用此仓库并提供反馈。要请求技术支持或提交增强请求，请[创建 GitHub issue](https://github.com/matlab/matlab-mcp-server/issues) 或联系 [MathWorks Technical Support](https://www.mathworks.com/support/contact_us.html)。
